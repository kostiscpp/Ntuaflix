...
with open(file_path, "w") as file:
  # Do nothing, just create the file
    ...
    for index, row in name_basics.iterrows():
        ...
        sql_query = f"INSERT INTO softeng.professional (primary_name, birth_date, death_date, image_url) VALUES ({primary_name}, {birth_date}, {death_date}, {image_url});\n"
    ...
    for index, row in name_basics.iterrows():
        ...
        for profession in primary_profession:
            sql_query = f"INSERT INTO softeng.professional_profession (professional_id, profession) VALUES ({professional_id}, '{profession}');\n"
    ...
    for index, row in title_basics.iterrows():
        ...
        sql_query = f"INSERT INTO softeng.media (title_type, primary_title, original_title, is_adult, start_year, end_year, runtime_minutes, poster_url, rating, no_of_ratings) VALUES ({titleType}, {primaryTitle}, {originalTitle}, {isAdult}, {startYear}, {endYear}, {runtimeMinutes}, {image_url}, {averageRating}, {numVotes});\n"
    ...
    for index, row in title_episode.iterrows():
        ...
        sql_query = f"INSERT INTO softeng.episode (episode_id, belongs_to, season_number, episode_number) VALUES ({title_id}, {parent_id} , {season_number}, {episode_number});\n"
        if test:
            print(sql_query)
        else:
            file.write(sql_query)
