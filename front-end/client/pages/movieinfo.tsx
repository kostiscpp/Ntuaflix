...
const MovieInfo = () => {
    ...
    const fetchMovieInfo = async () => {
        ...
        try {
            const response = await fetch(`https://localhost:8080/api/movieInfo/`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({movie: name}),
            });
        ...
        try {
            const response = await fetch(`https://localhost:8080/api/watchLater/`, {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({movie: name, token: localStorage.getItem('access_token')}),
            });
            const data = await response.json();
        ...
        try {
            const response = await fetch(`https://localhost:8080/api/watched/`, {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({movie: name, token: localStorage.getItem('access_token')}),
    ...
    const handleSubmit = async (e: any) => { 
        e.preventDefault();
        try {
            const response = await fetch('https://localhost:8080/api/addReview', {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({movie: name, rate: e.target.rating.value, text: e.target.review.value ?? "empty", token: localStorage.getItem('access_token')}),
            });
            const data = await response.json();
            if (data.success) {
                alert("Review added!");
...
<div className="mt-6">
        <button 
            className={`py-2 px-4 rounded text-white font-semibold ${hasWatched ? 'bg-blue-300 hover:bg-blue-400' : 'bg-blue-500 hover:bg-blue-700'}`}
            onClick={handleWatchedClick}
        >
            {hasWatched ? 'Watched ✔️' : 'Watched'}
        </button>
        <button
            className={`py-2 px-4 rounded text-white font-semibold ml-4 ${watchLater ? 'bg-blue-300 hover:bg-blue-400' : 'bg-blue-500 hover:bg-blue-700'}`} 
            ...
            {reviewResults.length > 0 && (
                reviewResults.map((review, index) => (
                    <div key={index} className="bg-blue-200 shadow rounded-lg p-4 mb-4">
                        <p className="font-semibold ">{review.rate}/10</p>
                        <p>{review.text}</p>
                    </div>
                ))
            )}
